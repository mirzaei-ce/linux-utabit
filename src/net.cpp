...
namespace {
    const int MAX_OUBTOUND_CONNECTIONS = 8;
...
static void AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    CAddress addr;
    int nInbound = 0;
    int nMaxInbound = nMaxConnections - MAX_OUBTOUND_CONNECTIONS;
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (semOutbound == NULL) {
        ...
        int nMaxOutbound = min(MAX_OUBTOUND_CONNECTIONS, nMaxConnections);
...
bool StopNode()
{
    ...
    MapPort(false);
    if (semOutbound)
        for (int i=0; i<MAX_OUBTOUND_CONNECTIONS; i++)
            semOutbound->post();
